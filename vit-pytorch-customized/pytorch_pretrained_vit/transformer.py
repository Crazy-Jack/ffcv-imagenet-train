...
class Transformer(nn.Module):
    ...
    def __init__(self, num_layers, dim, num_heads, ff_dim, dropout, topk_info):
        ...
        topk_sparsity = []
        if topk_info:
            # topk_layers is '0211232', each digit indicating whether the correponding layer is using 
            assert len(topk_info) == num_layers, f"len(topk_info) {len(topk_info)} doesn't match with num_layers {num_layers}. topk_info: {topk_info}, num_layers {num_layers}"
            while len(topk_info) != 0:
                topk_info, sparsity = topk_info[1:], topk_info[0]
                if sparsity != '0':
                    topk = float(sparsity) * 0.1
                else:
                    topk = 1.
                topk_sparsity.append(topk)
        else:
            assert topk_info == "" or topk_info is None
