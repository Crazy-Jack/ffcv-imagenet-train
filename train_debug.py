...
class ImageNetTrainer:
    ...
    def eval_and_log(self, extra_dict={}):
        if self.gpu == 0:
            ...
            start_val = time.time()
            shapebias_scores = run_evaluation(models.vgg16(pretrained=True)) # [('classname', 0.02),...]
            ...
            self.log(dict({
                ...
                'top_1': stats['top_1'],
                'top_5': stats['top_5'],
                'mean_shape_score': stats['mean_shape_score'],
                'std_shape_score': stats['std_shape_score'],
                'val_time': val_time
            }, **extra_dict))
        dist.barrier()  
    ...
    def val_loop(self, lr_tta):
        # shapebias_scores = ShapeBiasEval.run_evaluation(models.vgg16(pretrained=True)) # [('classname', 0.02),...]
        ...
        stats.update(
            {
                "shape_scores": shapebias_scores,
                "mean_shape_score": np.mean([i[1] for i in shapebias_scores]),
                "std_shape_score": np.std([i[1] for i in shapebias_scores])
            }
        )
