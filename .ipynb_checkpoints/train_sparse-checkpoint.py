...
from pytorch_pretrained_vit import ViT, ViT_sparse
...
Section('sparse', 'sparsify data options').params(
    linedrawing_threshold=Param(float, 'linedrawing threshold', default=0.5),
    line_drawing_path=Param(str, 'line_drawing_path', required=True),
    patch_selection_topk=Param(float, 'linedrawing threshold', default=0.10),
)
...
class ImageNetTrainer:
    ...
    @param('sparse.line_drawing_path')
    @param('sparse.patch_selection_topk')
    def create_model_and_scaler(self, arch, pretrained, distributed, use_blurpool, topk_info, resume_model_from_ckpt, model_ckpt, topk_layer_name, alexnet_topk, resnet50_topk, topk_tau, vgg_topk, topk_decay_ramp, line_drawing_path, patch_selection_topk):
        scaler = GradScaler()
        if 'vit' == arch.lower()[:3]:
            ...
            if model_name_vit == "S":
                ...
                model = ViT(pretrained=False,
                            ...
                            topk_info=topk_info)
            elif model_name_vit == "S_sparse":
                print(f"initializing vit-s sparse...")
                # vit small
                model = ViT_sparse(pretrained=False,
                            patches=16,
                            dim=384,
                            ff_dim=1536,
                            num_heads=6,
                            ...
                            topk_layer_name=topk_layer_name,
                            topk_info=topk_info,
                            line_drawing_ckpt=line_drawing_path,
                            patch_selection_topk=patch_selection_topk)
            else:
    ...
    @param('logging.log_level')
    @param('training.l1_sparsity_lamda')
    @param('sparse.linedrawing_threshold')
    def train_loop(self, epoch, log_level, l1_sparsity_lamda, linedrawing_threshold):
        model = self.model
        model.train()
        losses = []
