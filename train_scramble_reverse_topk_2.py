...
class ImageNetTrainer:
    ...
    def create_model_and_scaler(self, arch, pretrained, distributed, use_blurpool, topk_info, resume_model_from_ckpt, model_ckpt, topk_layer_name, alexnet_topk, resnet50_topk, topk_tau, vgg_topk):
        scaler = GradScaler()
    ...
    def set_topk_layer_reverse_state(self, model, set_revert_to):
        """
        this function set all topk's revert status
        """
        # iterate all named child that is topk layer?
        for name, m in model.module.features.named_children():
            if isinstance(m, TopKLayer):
                # print(f"Here is a topk layer's revert {name} revert={m.revert} before")
                m.revert = set_revert_to
                # print(f"Here is a topk layer's revert {name} revert={m.revert} After setting {set_revert_to}")
        

    @param('logging.log_level')
    @param('training.l1_sparsity_lamda')
    @param('training.scramble_reverse_weight')
    @param('training.scramble_reverse_lr_scale')
    def train_loop(self, epoch, log_level, l1_sparsity_lamda, scramble_reverse_weight, scramble_reverse_lr_scale):
        model = self.model
        model.train()
        losses = []
        ...
        for ix, (images, target) in enumerate(iterator):
            ...
            # normal train
            self.set_topk_layer_reverse_state(self.model, set_revert_to=False)
            ...
            # Revert training
            for param_group in self.optimizer.param_groups:
                param_group['lr'] = lrs[ix] * scramble_reverse_lr_scale
            self.set_topk_layer_reverse_state(self.model, set_revert_to=True)
            self.optimizer.zero_grad(set_to_none=True)
            with autocast():
                output = self.model(images)
                # print(f"output {output.shape}")
                # output_noise = torch.rand_like(output) * output.std(1).unsqueeze(1) + output.mean(1).unsqueeze(1)
                # output += output_noise
                target_perm_index = torch.randperm(len(target))
                target = target[target_perm_index]
                loss_revert_train = self.loss(output, target) * scramble_reverse_weight


            self.scaler.scale(loss_revert_train).backward()
            self.scaler.step(self.optimizer)
            self.scaler.update()
            ### Training end
            ...
            if log_level > 0:
                ...
                if log_level > 1:
                    names += ['loss', 'maximize loss_revert']
                    values += [f'{loss_train.item():.3f}', f'{loss_revert_train.item():.3f}']
    ...
    def val_loop(self, lr_tta):
        ...
        self.set_topk_layer_reverse_state(model, set_revert_to=False)
        with ch.no_grad():
            with autocast():
